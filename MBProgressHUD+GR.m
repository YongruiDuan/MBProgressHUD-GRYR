...
#define kContentBackFroundColor [UIColor colorWithWhite:0.f alpha:0.8f]
...
+ (void)alertTitle:(NSString *)title message:(NSString *)message cancel:(void(^)(void))cancel confirm:(void(^)(void))confirm {
	...
	if (confirm) {
		UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:CustomLocalizedString(@"confirmTitle", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
			confirm();
		}];
		[alertController addAction:confirmAction];
	}
	if (cancel && !confirm) {
		UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:CustomLocalizedString(@"confirmTitle", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
			cancel();
		}];
		...
		UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:CustomLocalizedString(@"cancelTitle", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
			cancel();
		}];
		[alertController addAction:cancelAction];
	}
