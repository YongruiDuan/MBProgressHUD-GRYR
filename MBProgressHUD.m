...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        if (appearance.progressTintColor == nil) {
			[(MBRoundProgressView *)indicator setProgressTintColor:[UIColor colorWithRGBHex:0xe65938]];//FIXME: YR添加的代码
//            ((MBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.5];
        }
